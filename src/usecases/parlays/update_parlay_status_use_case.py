...
class UpdateParlayStatusUseCase:
    ...
    async def __call__(self, event_status: EventStatusInput) -> None:
        parlays = await self.parlay_repository.find_by_event_token(event_status.token)
        for parlay in parlays:
            parlay.status = await self.__get_parlay_status(event_status.status)
